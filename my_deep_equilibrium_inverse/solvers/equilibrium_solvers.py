...
class EquilibriumProxGrad(nn.Module):
    ...
    def get_gradient(self, z, y):
        gramian = self.linear_op.gramian(z)
        return self.linear_op.gramian(z) - self._linear_adjoint(y)

    def forward(self, z, y):
        # z = 2 * (z - z.min()) / (z.max() - z.min()) - 1
        # print(f"THIS IS ETA:::::::::::::::::::::::::::::::::::{self.eta}")
        grad = self.get_gradient(z, y)
        # print(f"feed_z From {z.min()} to {z.max()}")

        gradstep = z - self.eta * grad
        # print(f"gradstep From {gradstep.min()} to {gradstep.max()}")
        
        # z_tplus1 = gradstep + self.nonlinear_op(z)
        # print(f"gradstep From {gradstep.min()} to {gradstep.max()}")
        z_tplus1 = self.nonlinear_op(gradstep)
        # print(f"nonlinear From {z_tplus1.min()} to {z_tplus1.max()}")
        


        # print(f"z_tplus1 From {z_tplus1.min()} to {z_tplus1.max()}")


        # print(f"nonlinear From {nonlinear.min()} to {nonlinear.max()}")
        # print("nonlinear", torch.isnan(nonlinear).sum(), torch.isinf(nonlinear).sum())
        
        # print(f"From {z_tplus1.min()} to {z_tplus1.max()}")
        # z_tplus1 = torch.clamp(z_tplus1, self.minval, self.maxval)


        # #Normalize
        # z_tplus1 = 2 * (z_tplus1 - z_tplus1.min()) / (z_tplus1.max() - z_tplus1.min()) - 1
        # print(f"From {z_tplus1.min()} to {z_tplus1.max()}")
        return z_tplus1
