...
def andersonexp(f, x0, m=5, lam=1e-4, max_iter=50, tol=1e-2, beta=1.0):
    ...
    X[:, 0], F[:, 0] = x0.reshape(bsz, -1), f(x0).reshape(bsz, -1)
    X[:, 1], F[:, 1] = F[:, 0], f(F[:, 0].reshape(x0.shape)).reshape(bsz, -1)
    print(f"F From {F.min()} to {F.max()}")
    print(f"X From {X.min()} to {X.max()}")
    ...
    for k in range(2, max_iter):
        ...
        H[:, 1:n + 1, 1:n + 1] = torch.bmm(G, G.transpose(1, 2)) + lam * torch.eye(n, dtype=x0.dtype, device=x0.device)[
            None]
        # alpha = torch.solve(y[:, :n + 1], H[:, :n + 1, :n + 1])[0][:, 1:n + 1, 0]  # (bsz x n)
        # print(f"H From {H.min()} to {H.max()}")
        # print(f"y From {y.min()} to {y.max()}")
        alpha = torch.linalg.solve(H[:, :n + 1, :n + 1], y[:, :n + 1])[:, 1:n + 1, 0]
        # print(f"alpha From {alpha.min()} to {alpha.max()}")
        X[:, k % m] = beta * (alpha[:, None] @ F[:, :n])[:, 0] + (1 - beta) * (alpha[:, None] @ X[:, :n])[:, 0]
        F[:, k % m] = f(X[:, k % m].reshape(x0.shape)).reshape(bsz, -1)
...
class DEQFixedPoint(nn.Module):
    def __init__(self, f, anderson, **kwargs):
        super().__init__()
        self.f = f
    ...
    def forward(self, x, initial_point = None):
        ...
        with torch.no_grad():
            z, self.forward_res = self.anderson(lambda z: self.f(z, x), init_point, **self.kwargs)
        ...
        def backward_hook(grad):
            ...
            # print(f"grad From {grad.min()} to {grad.max()}")
            # with torch.no_grad():
            print(f"Grad from {grad.min()} to {grad.max()}")

            func2 = lambda y: torch.autograd.grad(f0, z0, y, retain_graph=True)[0] + grad
            after2 = func2(grad)

            print(f"after grad 2 from {after2.min()} to {after2.max()}")
            g, self.backward_res = self.anderson(lambda y: torch.autograd.grad(f0, z0, y, retain_graph=True)[0],
                                               grad, **self.kwargs)
            
            # print(f"Grad from {grad.min()} to {grad.max()}")
            # print(f"g From {g.min()} to {g.max()}")
            return g
