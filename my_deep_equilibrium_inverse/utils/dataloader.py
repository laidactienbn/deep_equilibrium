...
class FolderDataset(Dataset):
    def __init__(self, target_directory, transform=None):
        filelist = os.listdir(target_directory)
        self.full_filelist = [os.path.join(target_directory, single_file) for single_file in filelist]
...
class MyDataset(Dataset):
    def __init__(self, target_directory, validation_data=False, transform=None):
        ...
        n_trainval = n_files * 8 // 10
        training_data = filelist[:n_train]
        val_data = filelist[n_train:n_trainval]
        test_data = filelist[n_trainval:]
        if validation_data:
            self.full_filelist = [os.path.join(target_directory, single_file) for single_file in val_data]
        else:
            self.full_filelist = [os.path.join(target_directory, single_file) for single_file in training_data]
...
class MyTestDataset(Dataset):
    def __init__(self, target_directory, transform=None):
        ...
        n_trainval = n_files * 8 // 10
        test_data = filelist[n_trainval:]
        self.full_filelist = [os.path.join(target_directory, single_file) for single_file in test_data]
        self.transform = transform
